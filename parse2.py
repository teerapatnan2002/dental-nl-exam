...
def categorize(text):
    ...
    if any(w in text_lower for w in ["perio", "pocket", "scrp", "bone loss", "gingiva", "plaque", "calculus", "pd "]):
        cat = "ปริทันตวิทยา"
    elif any(w in text_lower for w in ["caries", "อุด", "amalgam", "composite", "pulp", "dentin", "filling"]):
    ...
    elif any(w in text_lower for w in ["surgery", "extract", "fracture", "condylar", "mronj", "ถอน"]):
        ...
        cat = "ทันตกรรมชุมชน"
    elif any(w in text_lower for w in ["tmj", "muscle", "joint", "occlusal splint", "click", "masticat", "bupivacaine", "nerve"]):
        cat = "ทันตกรรมบดเคี้ยวและอาการปวดบริเวณช่องปากและใบหน้า"
    else:
        cat = "วิทยาการวินิจฉัยและเวชศาสตร์ช่องปาก"
        
    # Task
    if "treatment" in text_lower or "รักษา" in text_lower or "manage" in text_lower or "ทําอะไร" in text_lower or "ควรทำ" in text_lower or "tx." in text_lower:
        ...
        task = "การวินิจฉัยโรค"
    elif "สาเหตุ" in text_lower or "เกิดจาก" in text_lower or "mechanism" in text_lower or "เพราะ" in text_lower or "ผล" in text_lower or "ปัจจัย" in text_lower:
...
while i < len(lines):
    ...
    # Ignore standalone numbers like "66."
    ...
    if line.startswith("STEM ") or line.startswith("Stem "):
        ...
        while i < len(lines):
            ...
            if re.match(r'^' + str(next_expected_q) + r'\.\s+', l) or re.match(r'^' + str(next_expected_q) + r'\.\d+\s+', l) or (next_expected_q > 75 and re.match(r'^[1-4]\.\s+', l) and "Stem ปรศิ นา" in stem_text):
                break
            if l:
                stem_text += l + "\n"
            i += 1
    ...
    m_q = re.match(r'^(\d+(?:\.\d+)?)\.\s+(.*)', line)
    
    # We identify a question if it matches next_expected_q or if we are in Stem ปริศนา where numbering resets to 1.
    is_new_q = False
    if m_q:
        q_num_str = m_q.group(1)
        if q_num_str == str(next_expected_q):
            is_new_q = True
            next_expected_q += 1
        elif current_stem.startswith("Stem ปรศิ นา") and re.match(r'^[1-4](\.\d+)?$', q_num_str):
            is_new_q = True
    ...
    if is_new_q:
        ...
        # Read multi-line question text until we hit choices (which start with "1. ")
        i += 1
        while i < len(lines):
            l = lines[i].strip()
            if not l:
        ...
        # Backtrack 1 line to process the choice
        i -= 1
        
    elif in_choices and re.match(r'^([1-5])\.\s+(.*)', line):
        m_c = re.match(r'^([1-5])\.\s+(.*)', line)
        labels = {"1": "A", "2": "B", "3": "C", "4": "D", "5": "E"}
        choice_text = m_c.group(2).strip()
        
        # Look ahead for multiline choices
        i += 1
        while i < len(lines):
            l = lines[i].strip()
            if not l:
                i += 1
                continue
            # Stop if we hit next choice or next question
            if re.match(r'^[1-5]\.\s+', l) or re.match(r'^\d+\.\s+', l) or l.startswith("STEM") or l.startswith("Stem"):
                break
            choice_text += " " + l
            i += 1
...
# We need to write it to the actual requested output file using python itself as a shortcut, 
# because writing JSON dynamically directly from LLM could hit generation token limit.
# However, the user wants me to use `write_to_file` for the JSON.
# So this script will generate the string and then write it to the json file.
import json
json_str = json.dumps(out_data, ensure_ascii=False, indent=2)
# I will use a simple file write here first just to create it locally and then I'll read it and use write_to_file. Wait, I can just use python to write it and print success. But the user said: "Save as valid JSON to '/Users/admin/Downloads/NL Test/parsed_exams/NL_2_2566_Part_4.json' using write_to_file without markdown wrapping."
# To perfectly adhere, I will print it to stdout, read it in chunks, and then use `write_to_file`. No, stdout is better.
print(json_str)
